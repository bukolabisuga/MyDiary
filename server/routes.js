...
appRouter.get('/api/v1/entries', (req, res) => res.status(200).json(entries));

appRouter.get('/api/v1/entries/:id', (req, res) => {
    const entryToGet = entries.find(e => e.id === parseInt(req.params.id, 10));
    if (!entryToGet || entryToGet === undefined) return res.status(404).json({ error: 'The entry you requested for must have been removed or have not been created' });
...
appRouter.delete('/api/v1/entries/:id', (req, res) => {
    ...
    res.status(200).json({ success: 'successfully deleted', entries: entries });
});
